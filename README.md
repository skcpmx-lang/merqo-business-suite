...
| অংশ | যা পাবেন |
|---|---|
| **POS (বিক্রয়)** | বারকোড স্ক্যান (HID) দিয়ে দ্রুত বিক্রয়, অজানা বারকোড প্রবাহ, উজড় সীমা সতর্কতা, মেয়াদোত্তীর্ণ পণ্য ব্লক, ধাপে ধাপে পেমেন্ট (নগদ/ব্যাংক/MFS), বিক্রয় হোল্ড ও আবার চালু, রসিদ প্রিভিউ |
...
| **ক্রয়** | সাপ্লায়ারের কাছ থেকে পণ্য আসা, আংশিক পেমেন্ট/প্রদেয়, ফেরত (স্টকে ফেরত) |
| **কাস্টমার** | বকেয়া হিসাব, আদায়, উজড় সীমা সতর্কতা |
| **সাপ্লায়ার** | প্রদেয় হিসাব, পেমেন্ট |
| **খরচ** | ক্যাটাগরিভিত্তিক খরচ, কাস্টম ক্যাটাগরি |
| **হিসাব** | নগদ/ব্যাংক/MFS হিসাব, ট্রান্সফার, লিডজার |
| **শিফট** | ক্যাশিয়ার শিফট খোলা-বন্ধ, গণনা করা নগদ বনাম হিসাবমত, ফারাক |
| **MFS এজেন্ট** | bKash/Nagad এজেন্ট হিসাব — ক্যাশ-ইন/আউট, সেন্ড মানি, কমিশন, রিকনসিলিয়েশন (ম্যানুয়াল রেকর্ডিং, কোনো লাইভ API নেই) |
...
| **ব্যবহারকারী** | ভূমিকা (মালিক/অ্যাডমিনিস্ট্রেটর/ম্যানেজার/ক্যাশিয়ার/…), অধিকার, পাসওয়ার্ড, মুছে ফেলা যায় না এমন অডিট লগ |
| **নিরাপত্তা** | সব লেনদেন এক ট্রানজেকশনে — আধা অবস্থা সম্ভব নয়; ডাবল-সাবমিশন সুরক্ষা; টাকা = পাইসা (ফ্লোট নয়) |
...
## স্ট্যাটাস ও ডকুমেন্টেশন

- `docs/QA_REPORT.md` — রিলিজ-গেট স্ট্যাটাস (ফেজ-ভিত্তিক PASS / FAIL / NOT TESTED)
- `docs/BANGLA_GLOSSARY.md` — ক্যাননিকাল বাংলা পদাভিধান
- `docs/RELEASE_NOTES.md` — v1.0.0 নোট

Windows প্রোডাকশন বিল্ড (.exe/ইনস্টলার), প্রিন্টার, স্ক্যানার, DPI-চেক —
এগুলো আসল Windows হার্ডওয়্যারে যাচাই না হওয়া পর্যন্ত রিলিজ-গেট **NOT MET**
(বিস্তারিত `docs/QA_REPORT.md`-এ)।
...
│                              migrations 0001→0004 (versioned)  │
└────────────────────────────────────────────────────────────────┘
```

- **স্তরবদ্ধ ডিজাইন**: UI → IPC → service → pure-TS domain → repository → SQLite।
  Domain স্তর Electron-মুক্ত — টেস্টে সরাসরি চলে।
- **atomic লেনদেন**: বিক্রয় = স্টক কমানো + হিসাব বাড়ানো + লেনদেন লেখা — এক
  ট্রানজেকশনে; ব্যর্থ হলে সব বাতিল (rollback)।
- **টাকা**: সব সংখ্যা পিসায় (integer) — ৳ 12,500.00 ফরম্যাট, কোনো ফ্লোটিং-পয়েন্ট নয়।
- **অডিট**: গুরুত্বপূর্ণ সব কাজ (বিল বাতিল, স্টক সমন্বয়, পুনরুদ্ধার…) অডিট টেবিলে
  লেখা হয় — মুছে ফেলা যায় না।
- **মাইগ্রেশন**: সংস্করণ-সহ স্কিমা আপগ্রেড; পুরোনো ডাটাবেস নিরাপদে খোলে।
